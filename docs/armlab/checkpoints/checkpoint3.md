...
Checkpoint 2 gave you two things: a forward kinematics you trust, and a camera that can turn a pixel into a point on the board. This checkpoint closes the loop between them. First inverse kinematics, so that a point in the world becomes a set of joint angles. Then a block detector, so that the camera finds the blocks rather than you clicking on them. Finally, the two together: click a block, and the arm picks it up, click another block or empty space and the arm places it down.
...
1. **Find the wrist center.** The flange sits a fixed distance along the final joint axis from where the three wrist axes meet. Take the target position, and step back along the target's tool axis by that distance. You now have a point whose position depends only on joints 1, 2 and 3.
2. **Solve joint 1** from where the wrist center sits when viewed from above. On this arm there is no lateral shoulder offset, so this is a single `atan2`.
...
- The law of cosines tells you about reachability before it tells you an angle: if the cosine it produces is outside `[−1, 1]`, the wrist center is beyond the arm's reach. That is your first `None`.
- **Singularities.** When joint 5 is zero, joints 4 and 6 spin about the same axis and only their sum is determined. Detect the case and choose one, for instance keep joint 4 at zero, rather than dividing by something that is nearly zero. When the wrist center lies on the joint 1 axis, joint 1 is undefined in the same way.
...
Implement `IK_numerical(dh_params, pose, q0, joint_limits, w_rot)` as **bounded Gauss-Newton least squares**: iteratively minimize the error between `FK(q)` and the target pose, keeping every joint inside its limits.
...
- Plain Gauss-Newton can overshoot when the Jacobian is nearly singular, which is exactly what happens near the wrist singularity. Adding a small damping term to the normal equations (the Levenberg-Marquardt idea) costs one line and removes most of the bad behavior.
...
{: .sanity_check}
For a reachable pose, the numerical solver converges in a handful of iterations and `FK` of its answer matches the target to well under a millimeter. For one just out of reach it returns `None` instead of a nearby pose.
...
1. Choose the flange pose for a top-down grasp of a 38 mm block on the table: tool pointing straight down, flange 85 mm above the block's center height.
...
Everything so far has needed you to click. Now the camera does the finding. The approach follows the lecture: get to a top-down view of the board where the table is flat, threshold height to find the blocks, and only then look at color.
...
Write the detector as a set of functions in `src/camera.py`, run from `VideoThread` in the same way `update_workspace_frame()` is. There is no stub. Each detection should end up as a record with the block's **world position**, **color**, **size** and **orientation**, and the results should be drawn on a view in the GUI so you can see what the detector sees.

**1. Depth in the top-down view**

You already warp the RGB image into the Workspace view. Warp the depth image with the same homography. Use nearest-neighbor interpolation for depth: bilinear would average a valid reading with an invalid zero, or across a block edge, and invent depths that exist nowhere.
...
`cv2.minAreaRect` gives a rotated rectangle: its center, its side lengths and its angle. In the top-down view the pixel scale is uniform, and you know it from the tag spacing you used to build the homography, so side lengths convert straight to millimeters. That separates 38 mm blocks from 25 mm ones. The angle is the block's orientation, and since blocks are square it only matters modulo 90°.
...
**6. Color from the RGB image**

Color comes last, and only from pixels you already know belong to a block. Erode the contour mask a little so the sample does not include the block's edges, then take the median color inside it. Classify in HSV by hue. Red wraps around zero, so it needs two ranges. Tune the ranges under the lab's actual lighting, on all six colors, and write the numbers down, because they are part of your method.
...
The rectangle's center is a pixel in the top-down view. Send it back through the inverse homography to the original image, read the depth there, and run it through `image_to_world`. Use the **original** depth for this, not the flattened one: flattening was for thresholding, and it changed the numbers. Because the homography is exact on the table plane, a block's `x` and `y` from the top-down view and from the original view agree to within a millimeter.

**Hints**

- You can develop on saved image pairs first, in a standalone script, with the intermediate images displayed. Once it works, move the functions into `camera.py`. Debugging inside the live GUI is slow.
- Draw every intermediate: the warped depth, the flattened depth, the mask, the contours, the rectangles. Most detector bugs are visible in one of these images.
- Blocks touching each other merge into one contour. You do not need to solve that for this checkpoint, but notice when it happens, we will not purposefully do this for the competition but it may happen by accident, and it is detectable.
- If you have issues with changing lighting making color detection difficult, you may want to have a color calibration step where you capture the color ranges automatically.

{: .sanity_check}
With several blocks of different colors and both sizes on the board, the view shows each one outlined, with its color, size and orientation labeled and its world coordinates printed, and none of the tags, the arm, or the empty table is labeled as a block.

## Task 3.5  Measure the detector

A detector you have not measured is a demo. Produce data that show your detector functions.

1. **Position.** Place blocks at grid intersections you know, across the whole board, and compare detected against true position. Report the error and how it varies with distance from the center.
2. **Color.** Show the detector every color, several times each, at several places on the board. Build a confusion matrix: which colors it gets right, and which it confuses with which.
...
{: .sanity_check}
You can state the detector's accuracy as numbers with regions attached, and you know which color pair it confuses most and under what lighting.
...
2. Compute the grasp pose: tool pointing down at that point, at the height of the block's center. Move the target 85 mm up to get the flange pose.
...
- Perform motions slowly at first.
- Use `wait=True` on every move so the sequence runs in order, and remember the gripper call returns before the jaws have finished traveling, so time your gripper and add a delay.
...
{: .submission}
**1)** Your geometric IK derivation: the wrist-center decoupling, the equations for each joint, and a schematic of the arm marking the wrist center and the two-link triangle. <br>
...
**4)** Your detector measurements from Task 3.5: the position error plot and the color confusion matrix. <br>
**5)** A short video of click-to-grab and click-to-place, showing both the control station and the real arm.
