...
This is the address you set as `XARM_IP` during [setup](/docs/armlab/setup-guide#4-set-your-stations-arm-ip).

{: .note}
If the connection fails, `ping` the address from a terminal before touching your code. No reply means a cable, IP or arm problem, not a Python problem. See [Linux Command Line Tools](/docs/armlab/how-to-guide/linux-clt#checking-hardware-and-the-network).
...
The gripper is custom-built for this lab (see [Hardware](/docs/armlab/hardware#gripper--end-effector)) but is driven by the standard Lite 6 gripper calls:
