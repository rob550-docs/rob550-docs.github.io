...
- Your station is set up and the control station runs. If not, work through the [Setup Guide](/docs/new-armlab/setup-guide) first.
- You know where the emergency stop is. It is on the table next to the arm.
- You have read the [Hardware](/docs/new-armlab/hardware) page, in particular the joint limits and the 600 g payload.
...
1. Find the **joint jog** controls and move each of the six joints one at a time with the sliders. Watch which physical joint moves, and compare the direction of travel to the joint numbering on the [Hardware](/docs/new-armlab/hardware#ufactory-lite-6-arm) page.
...
Now switch to the codebase you will use for the rest of the lab. Start the control station and spend a few minutes with the interface: the video panel and its four views, the joint readouts and Direct Control. Read [Software](/docs/new-armlab/software) alongside it so you know which file each panel talks to.
...
- Joint angles in this codebase are **radians**. The GUI converts to degrees for display only — see [Units and conventions](/docs/new-armlab/software#units-and-conventions).
